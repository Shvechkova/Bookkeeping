...
function getInfoOperAccountOperation(element) {
  ...
  const dataName = element.getAttribute("data-operation-data-start");
...
function getSumOldOperAccountOperation(element, elem) {
  ...
  const startDate = element.getAttribute("data-operation-data-start-all");
  let bank;
  if (bankElem) {
  ...
  form.append("operaccount", true);
  form.append("start_date", startDate);
  form.append("old", true);
  let object = {};
      ...
      fetch("/api/v1/operation/operation_out_filter/", {
        ...
        .then((data) => {
          ...
          if (data.length > -1) {
            ...
            DeOperAccountlOperation(element);
            console.log(st);
...
function DeOperAccountlOperation(element) {
