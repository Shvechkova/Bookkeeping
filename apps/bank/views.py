...
from apps.core.utils import (
    ...
    fill_operations_storage_servise,
    fill_operations_storage_servise_client,
    get_id_categ_oper,
...
def salary(request):
    ...
    operations = (
        ...
        .select_related("salary", "employee")  # Добавляем select_related для employee
        .prefetch_related()
        .order_by("-data")
    )
    ...
    # Структура для хранения данных по сотрудникам
    employees_data = {}

    # Инициализируем структуру данных для каждого сотрудника
    # Создаем словарь с группами категорий и месяцами
    ...
    # Распределяем операции по сотрудникам, месяцам и группам
    for operation in operations:
...
def storage_servise_client(request):
    ...
    context = {
        ...
        "type_url": type_url,
        "bank": bank.id,
        "arr_clients_all": arr_clients_all,
        "months_current_year": months_current_year,
    }
